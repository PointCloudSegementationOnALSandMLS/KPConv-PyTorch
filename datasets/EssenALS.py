...
class EssenALSDataset(PointCloudDataset):
    """Class to handle EssenALS dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'EssenALS')
        ...
        # Dataset folder
        self.path = '../Essen/aerial/normalized'
        ...
        self.all_splits = [0,1,2,3,4,5,0,6,7,8,0,0]
        self.validation_split = 0
        self.test_splits = 0
        self.train_splits = [1,2,3,4,5,6,7,8]
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for EssenALS (with features) data: ', self.set)
...
class EssenALSSampler(Sampler):
    """Sampler for EssenALS (with features)"""

    def __init__(self, dataset: EssenALSDataset):
...
class EssenALSCustomBatch:
    """Custom batch definition with memory pinning for EssenALS (with features)"""
...
def EssenALSCollate(batch_data):
    return EssenALSCustomBatch(batch_data)
