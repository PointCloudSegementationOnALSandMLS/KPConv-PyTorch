...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling Essen_MLS with geometric features) dataset.
...
class EssenMLSDataset(PointCloudDataset):
    """Class to handle EssenMLS dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'EssenMLS')
        ...
        # Dataset folder
        self.path = '../Essen/mobile/normalized'
        ...
        self.cloud_names = ["grid_0","grid_1","grid_2","grid_3","grid_4","grid_5","grid_6","grid_7","grid_8","grid_9", "grid_10", "test_grid"]
        self.all_splits = [0,1,2,3,4,5,0,7,8,9,0,0]
        self.validation_split = 0
        self.test_splits = 0
        self.train_splits = [1,2,3,4,5,7,8,9]

        # Define offset
        self.UTM_OFFSET = [627285, 4841948, 0]

        self.test_cloud_names = ["grid_0","grid_6", "grid_10", "test_grid"]
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for EssenMLS (with features) data: ', self.set)
    ...
    def prepare_EssenMLS_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            data = np.load(join(ply_path, cloud_name + '.npy'))
            
            xyz = np.array(data[:, :3], dtype=np.float32)
            kdtree = cKDTree(xyz)
            ...
            for i in range(num_points):
                ...
                _, neighbor_index = kdtree.query(xyz[i], k=2)
                
                # Skip the first neighbor (itself), take the second neighbor
                point1 = xyz[i]
                point2 = xyz[neighbor_index[1]]
            ...
            non_isolated_points = np.delete(data, isolated_points, axis=0)

            xyz =  np.array(non_isolated_points[:, :3], dtype=np.float32)
            labels = np.array(non_isolated_points[:, 3], dtype=np.uint8)
            feat = np.array(non_isolated_points[:, 4], dtype=np.uint8)

            # Save as ply
            write_ply(join(ply_path, cloud_name + '.ply'),
                    (xyz, feat, labels),
                    ['x', 'y', 'z',  'scalar_Intensity', 'scalar_Label'])
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                sub_colors = np.vstack(data['scalar_Intensity']).T.reshape((len(data['scalar_Intensity']), 1))
                print(len(data['scalar_Intensity']))
                print(sub_colors.shape)
                ...
                colors = (data['scalar_Intensity']).T.reshape((len(data['scalar_Intensity']), 1))
...
class EssenMLSSampler(Sampler):
    """Sampler for EssenMLS (with features)"""

    def __init__(self, dataset: EssenMLSDataset):
...
class EssenMLSCustomBatch:
    """Custom batch definition with memory pinning for EssenMLS (with features)"""
...
def EssenMLSCollate(batch_data):
    return EssenMLSCustomBatch(batch_data)
